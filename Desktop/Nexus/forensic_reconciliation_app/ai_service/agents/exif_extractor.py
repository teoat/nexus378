...
class EXIFExtractor:
    ...
    async def extract_metadata(self, file_path: str) -> EXIFMetadata:
        ...
        try:
            ...
            self.logger.info(f"EXIF metadata extraction completed: {exif_metadata.metadata_id}")
    ...
    async def _extract_standard_exif(self, file_path: str, image_format: ImageFormat) -> Dict[str, Any]:
    ...
    def _extract_camera_info(self, tags: Dict) -> Optional[CameraInfo]:
        ...
        try:
            ...
            firmware = str(tags.get('EXIF FirmwareVersion', '')) if 'EXIF FirmwareVersion' in tags else None
            serial_number = str(tags.get('EXIF BodySerialNumber', '')) if 'EXIF BodySerialNumber' in tags else None
            
            lens_make = str(tags.get('EXIF LensMake', '')) if 'EXIF LensMake' in tags else None
            lens_model = str(tags.get('EXIF LensModel', '')) if 'EXIF LensModel' in tags else None
            lens_serial = str(tags.get('EXIF LensSerialNumber', '')) if 'EXIF LensSerialNumber' in tags else None
    ...
    def _extract_gps_data(self, tags: Dict) -> Optional[GPSData]:
        ...
        try:
            ...
            if 'GPS GPSLatitude' in tags and 'GPS GPSLongitude' in tags:
                latitude = self._convert_gps_coordinate(tags['GPS GPSLatitude'], tags.get('GPS GPSLatitudeRef', 'N'))
                longitude = self._convert_gps_coordinate(tags['GPS GPSLongitude'], tags.get('GPS GPSLongitudeRef', 'E'))
            
            if 'GPS GPSAltitude' in tags:
                altitude = float(tags['GPS GPSAltitude'].num) / float(tags['GPS GPSAltitude'].den)
                if 'GPS GPSAltitudeRef' in tags and tags['GPS GPSAltitudeRef'].num == 1:
                    altitude = -altitude
            
            # GPS references
            latitude_ref = str(tags.get('GPS GPSLatitudeRef', '')) if 'GPS GPSLatitudeRef' in tags else None
            longitude_ref = str(tags.get('GPS GPSLongitudeRef', '')) if 'GPS GPSLongitudeRef' in tags else None
            altitude_ref = str(tags.get('GPS GPSAltitudeRef', '')) if 'GPS GPSAltitudeRef' in tags else None
            ...
            if 'GPS GPSTimeStamp' in tags and 'GPS GPSDateStamp' in tags:
                try:
                    ...
                    gps_timestamp = datetime.strptime(f"{date_str} {time_parts[0]}:{time_parts[1]}:{time_parts[2]}", '%Y:%m:%d %H:%M:%S')
                except Exception:
                    pass
            
            # GPS version
            gps_version = str(tags.get('GPS GPSVersionID', '')) if 'GPS GPSVersionID' in tags else None
    ...
    def _extract_timestamp_info(self, tags: Dict) -> Optional[TimestampInfo]:
        ...
        try:
            ...
            if 'EXIF DateTimeOriginal' in tags:
                try:
                    original_date = datetime.strptime(str(tags['EXIF DateTimeOriginal']), '%Y:%m:%d %H:%M:%S')
                except Exception:
                    pass
            ...
            if 'EXIF DateTimeDigitized' in tags:
                try:
                    digitized_date = datetime.strptime(str(tags['EXIF DateTimeDigitized']), '%Y:%m:%d %H:%M:%S')
                except Exception:
                    pass
            ...
            if 'Image DateTime' in tags:
                try:
                    modified_date = datetime.strptime(str(tags['Image DateTime']), '%Y:%m:%d %H:%M:%S')
                except Exception:
                    pass
            
            # Subsecond time
            subsec_time = str(tags.get('EXIF SubSecTime', '')) if 'EXIF SubSecTime' in tags else None
    ...
    def _extract_technical_data(self, tags: Dict) -> Optional[TechnicalData]:
        ...
        try:
            # Image dimensions
            image_width = int(tags['EXIF ExifImageWidth']) if 'EXIF ExifImageWidth' in tags else None
            image_height = int(tags['EXIF ExifImageLength']) if 'EXIF ExifImageLength' in tags else None
            
            # Orientation
            orientation = int(tags['Image Orientation']) if 'Image Orientation' in tags else None
            
            # Color space
            color_space = str(tags.get('EXIF ColorSpace', '')) if 'EXIF ColorSpace' in tags else None
            
            # Bits per sample
            bits_per_sample = int(tags['EXIF BitsPerSample']) if 'EXIF BitsPerSample' in tags else None
            
            # Compression
            compression = str(tags.get('Image Compression', '')) if 'Image Compression' in tags else None
            ...
            if 'Image XResolution' in tags:
                ...
                x_resolution = float(x_res.num) / float(x_res.den) if hasattr(x_res, 'num') else float(x_res)
            
            if 'Image YResolution' in tags:
                y_res = tags['Image YResolution']
                y_resolution = float(y_res.num) / float(y_res.den) if hasattr(y_res, 'num') else float(y_res)
            
            resolution_unit = str(tags.get('Image ResolutionUnit', '')) if 'Image ResolutionUnit' in tags else None
            
            # Exposure settings
            exposure_time = None
            if 'EXIF ExposureTime' in tags:
                exp_time = tags['EXIF ExposureTime']
                exposure_time = float(exp_time.num) / float(exp_time.den) if hasattr(exp_time, 'num') else float(exp_time)
            
            f_number = None
            if 'EXIF FNumber' in tags:
                f_num = tags['EXIF FNumber']
                f_number = float(f_num.num) / float(f_num.den) if hasattr(f_num, 'num') else float(f_num)
            
            iso_speed = int(tags['EXIF ISOSpeedRatings']) if 'EXIF ISOSpeedRatings' in tags else None
            ...
            if 'EXIF FocalLength' in tags:
                ...
                focal_length = float(focal.num) / float(focal.den) if hasattr(focal, 'num') else float(focal)
            
            # White balance
            white_balance = str(tags.get('EXIF WhiteBalance', '')) if 'EXIF WhiteBalance' in tags else None
            
            # Metering mode
            metering_mode = str(tags.get('EXIF MeteringMode', '')) if 'EXIF MeteringMode' in tags else None
    ...
    def _extract_editing_info(self, tags: Dict) -> Optional[EditingInfo]:
        ...
        try:
            software = str(tags.get('Image Software', '')) if 'Image Software' in tags else None
            artist = str(tags.get('Image Artist', '')) if 'Image Artist' in tags else None
            copyright = str(tags.get('Image Copyright', '')) if 'Image Copyright' in tags else None
            user_comment = str(tags.get('EXIF UserComment', '')) if 'EXIF UserComment' in tags else None
    ...
    async def _extract_heic_metadata(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            metadata = {
                ...
                'technical_data': None,
                'editing_info': None,
                'raw_exif': {'format': 'HEIC', 'note': 'HEIC metadata extraction not implemented'}
            }
    ...
    async def _extract_basic_metadata(self, file_path: str, image_format: ImageFormat) -> Dict[str, Any]:
        ...
        try:
            metadata = {
                ...
                'technical_data': None,
                'editing_info': None,
                'raw_exif': {'format': image_format.value, 'note': 'Basic metadata only'}
            }
    ...
    async def _cleanup_old_metadata(self):
        ...
        while True:
            try:
                ...
                cutoff_time = current_time - timedelta(days=90)  # Keep 3 months of metadata
                
                # Clean up old metadata
                old_metadata = [
                    metadata_id for metadata_id, metadata in self.extracted_metadata.items()
                ...
                if old_metadata:
                    self.logger.info(f"Cleaned up {len(old_metadata)} old metadata entries")
        ...
        while True:
            try:
                ...
                if self.total_extractions > 0:
                    total_time = sum(
                        metadata.extraction_time for metadata in self.extracted_metadata.values()
                    )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'extraction_statuses_supported': [status.value for status in ExtractionStatus],
...
if __name__ == "__main__":
    # Configuration
    config = {
        'supported_formats': ['jpeg', 'jpg', 'png', 'tiff', 'tif', 'gif', 'bmp', 'webp'],
