...
class FraudAgentEntityNetwork:
    ...
    async def analyze_entity_network(
        self, entities: List[Dict[str, Any]], transactions: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Analyze entity network for fraud patterns"""
        try:
            logger.info(
                f"Analyzing entity network with {len(entities)} entities and {len(transactions)} transactions"
            )
    ...
    async def _build_entity_network(
        ...
        try:
            ...
            relationship_weights = defaultdict(
                lambda: {
                    "count": 0,
                    "total_amount": 0.0,
                    "first_time": None,
            ...
            for tx in transactions:
                ...
                if from_entity and to_entity and from_entity != to_entity:
                    ...
                    if (
                        relationship_weights[key]["first_time"] is None
                        or timestamp < relationship_weights[key]["first_time"]
                    ):
                        relationship_weights[key]["first_time"] = timestamp

                    if (
                        relationship_weights[key]["last_time"] is None
                        or timestamp > relationship_weights[key]["last_time"]
                    ):
            ...
            for (entity1, entity2), weight_data in relationship_weights.items():
                if weight_data["count"] >= self.config["minimum_transaction_count"]:
                    strength = min(1.0, weight_data["total_amount"] / 100000)
            ...
            logger.info(
                f"Built network with {self.entity_graph.number_of_nodes()} nodes and {self.entity_graph.number_of_edges()} edges"
            )
        ...
        try:
            ...
            for entity_id, entity in self.entities.items():
                if entity.entity_type in ["company", "corporation", "llc"]:
                    ...
                    if (
                        shell_indicators.shell_score
                        >= self.config["shell_company_threshold"]
                    ):
    ...
    async def _analyze_shell_indicators(
            ...
            if incorporation_date:
                try:
                    ...
                    if age_years < self.config["shell_indicators"]["max_age_years"]:
                        indicators.append(
                            f"Very new company: {age_years:.1f} years old"
                        )
                        score_factors.append(0.15)
                except:
            ...
            if entity_id in self.entity_graph:
                ...
                if degree == 2:
                    ...
                    if len(neighbors) == 2:
                        ...
                        amount_ratio = min(
                            edge1_data.get("total_amount", 0),
                            edge2_data.get("total_amount", 0),
                        ) / max(
                            edge1_data.get("total_amount", 1),
                            edge2_data.get("total_amount", 1),
                        )
        ...
        try:
            ...
            try:
                eigenvector_centrality = nx.eigenvector_centrality(
                    self.entity_graph, max_iter=1000
                )
            except:
            ...
            centrality_analysis = {
                ...
                "largest_component_size": (
                    len(max(nx.connected_components(self.entity_graph), key=len))
                    if nx.number_connected_components(self.entity_graph) > 0
                    else 0
                ),
            }
        ...
        try:
            ...
            try:
                partition = community_louvain.best_partition(
                    self.entity_graph,
                    resolution=self.config["community_detection_resolution"],
                )
            except:
                # Fallback to simple connected components
                partition = {}
                for i, component in enumerate(
                    nx.connected_components(self.entity_graph)
                ):
            ...
            for community_id, entity_list in community_groups.items():
                if len(entity_list) >= 3:  # Minimum community size
                    community_analysis = await self._analyze_community(
                        entity_list, community_id
                    )
    ...
    async def _analyze_community(self, entity_list: List[str], community_id: int):
        ...
        try:
            ...
            if "bank" in type_counter:
                ...
                "company" in type_counter
                and type_counter["company"] > len(entity_list) * 0.7
            ):
            ...
            high_risk_entities = sum(
                ...
                for entity_id in entity_list
                if entity_id in self.entities
                and self.entities[entity_id].risk_score > 0.7
            )
    ...
    def _calculate_network_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics = {
                ...
                "average_clustering": nx.average_clustering(self.entity_graph),
                "average_degree": sum(dict(self.entity_graph.degree()).values())
                / self.entity_graph.number_of_nodes(),
    ...
    def get_network_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_risk_shell_companies": len(
                [sc for sc in self.shell_companies if sc.risk_level == "HIGH"]
            ),
            "communities": len(self.communities),
            "high_risk_communities": len(
                [c for c in self.communities if c.risk_level == "HIGH"]
            ),
            "network_metrics": self._calculate_network_metrics(),
