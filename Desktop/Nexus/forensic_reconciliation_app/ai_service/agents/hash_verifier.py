...
class HashVerifier:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Configuration
        self.default_algorithms = config.get('default_algorithms', [
            HashAlgorithm.MD5,
    ...
    async def calculate_file_hashes(self, file_path: str, algorithms: List[HashAlgorithm] = None) -> Dict[HashAlgorithm, HashResult]:
        ...
        try:
            ...
            if self.enable_parallel_processing:
                ...
                for algorithm in algorithms:
                    task = asyncio.create_task(self._calculate_hash_parallel(file_path, algorithm, file_size))
                    tasks.append(task)
                ...
                for i, result in enumerate(results):
                    if isinstance(result, Exception):
                        self.logger.error(f"Error calculating {algorithms[i].value} hash: {result}")
                    else:
                ...
                for algorithm in algorithms:
                    try:
                        hash_result = await self._calculate_hash_sequential(file_path, algorithm, file_size)
                        hash_results[algorithm] = hash_result
                    except Exception as e:
                        self.logger.error(f"Error calculating {algorithm.value} hash: {e}")
            
            self.logger.info(f"Hash calculation completed for {len(hash_results)} algorithms")
    ...
    async def _calculate_hash_parallel(self, file_path: str, algorithm: HashAlgorithm, file_size: int) -> HashResult:
    ...
    async def _calculate_hash_sequential(self, file_path: str, algorithm: HashAlgorithm, file_size: int) -> HashResult:
    ...
    async def verify_file_integrity(self, file_path: str, expected_hashes: Dict[HashAlgorithm, str],
                                   integrity_level: IntegrityLevel = IntegrityLevel.ENHANCED) -> VerificationResult:
        ...
        try:
            ...
            for algorithm, expected_hash in expected_hashes.items():
                if algorithm in calculated_hashes:
                    ...
                    if calculated_hash.hash_value.lower() != expected_hash.lower():
                        ...
                        errors.append(f"Hash mismatch for {algorithm.value}: expected {expected_hash}, got {calculated_hash.hash_value}")
                else:
            ...
            verification_time = sum(hash_result.calculation_time for hash_result in calculated_hashes.values())
            ...
            self.logger.info(f"File integrity verification completed: {verification_id} - Status: {verification_status.value}")
    ...
    async def add_hash_to_database(self, hash_value: str, algorithm: HashAlgorithm,
                                  file_info: str, source: str) -> str:
    ...
    async def search_hash_database(self, hash_value: str, algorithm: HashAlgorithm = None) -> List[HashDatabase]:
    ...
    async def verify_hash_against_database(self, file_path: str, algorithm: HashAlgorithm = None) -> List[HashDatabase]:
        ...
        try:
            ...
            for calc_algorithm, hash_result in calculated_hashes.items():
                if algorithm is None or calc_algorithm == algorithm:
                    database_matches = await self.search_hash_database(hash_result.hash_value, calc_algorithm)
                    matches.extend(database_matches)
            
            self.logger.info(f"Database verification completed: {len(matches)} matches found")
    ...
    async def generate_hmac(self, file_path: str, secret_key: str, algorithm: HashAlgorithm = HashAlgorithm.SHA256) -> str:
        ...
        try:
            ...
            hmac_obj = hmac.new(secret_key.encode(), file_content, hashlib.new(algorithm.value))
    ...
    async def verify_hmac(self, file_path: str, expected_hmac: str, secret_key: str,
    ...
    async def generate_checksum_file(self, directory_path: str, output_file: str,
                                   algorithms: List[HashAlgorithm] = None) -> str:
            ...
            for root, dirs, files in os.walk(directory_path):
                for filename in files:
                    ...
                    try:
                        ...
                        file_hashes = await self.calculate_file_hashes(file_path, algorithms)
    ...
    async def verify_checksum_file(self, checksum_file: str, directory_path: str) -> Dict[str, VerificationResult]:
        ...
        try:
            ...
            for relative_path, expected_hashes in checksum_data.items():
                ...
                if os.path.exists(file_path):
                    ...
                    if algorithms:
                        # Verify file
                        verification_result = await self.verify_file_integrity(file_path, expected_hash_dict)
                        verification_results[relative_path] = verification_result
                else:
                    self.logger.warning(f"File not found: {file_path}")
            
            self.logger.info(f"Checksum verification completed: {len(verification_results)} files verified")
    ...
    async def _cleanup_old_results(self):
        ...
        while True:
            try:
                ...
                cutoff_time = current_time - timedelta(days=30)  # Keep 30 days of results
                
                # Clean up old results
                old_results = [
                    verification_id for verification_id, result in self.verification_results.items()
                ...
                if old_results:
                    self.logger.info(f"Cleaned up {len(old_results)} old verification results")
        ...
        while True:
            try:
                ...
                if self.total_verifications > 0:
                    total_time = sum(
                        result.verification_time for result in self.verification_results.values()
                    )
                    self.average_verification_time = total_time / self.total_verifications
        ...
        try:
            ...
            for algorithm in HashAlgorithm:
                try:
                    ...
                    hash_obj.hexdigest()
                    self.logger.debug(f"Hash algorithm {algorithm.value} initialized successfully")
                except Exception as e:
                    self.logger.warning(f"Hash algorithm {algorithm.value} not available: {e}")
    ...
    def get_verification_status(self, verification_id: str) -> Optional[VerificationResult]:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'verification_statuses_supported': [status.value for status in VerificationStatus],
