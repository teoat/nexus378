...
class PatternDetector:
        ...
        try:
            ...
            for pattern_type in pattern_types:
                try:
                    ...
                    self.logger.error(f"Error detecting {pattern_type.value} patterns: {e}")
                    continue
            ...
            self.logger.info(f"Detected {len(filtered_patterns)} patterns in {processing_time:.2f}s")
    ...
    async def _detect_transaction_patterns(self, data: Dict[str, Any]) -> List[DetectedPattern]:
            ...
            if 'amount' in df.columns:
                ...
                if len(high_value_transactions) > 0:
                    pattern = DetectedPattern(
                        ...
                        evidence={'threshold': high_value_threshold, 'count': len(high_value_transactions)},
                        risk_score=0.6,
                ...
                if std_amount > 0:
                    ...
                    if len(unusual_transactions) > 0:
                        pattern = DetectedPattern(
                            ...
                            entities_involved=unusual_transactions['entity_id'].tolist(),
                            evidence={'z_scores': z_scores[z_scores > 3].tolist(), 'count': len(unusual_transactions)},
                            risk_score=0.75,
                            timestamp=datetime.utcnow()
                        )
                        patterns.append(pattern)
            ...
            if 'entity_id' in df.columns:
                ...
                if len(high_frequency_entities) > 0:
                    pattern = DetectedPattern(
                        ...
                        evidence={'frequency_counts': high_frequency_entities.to_dict()},
                        risk_score=0.5,
                        timestamp=datetime.utcnow()
                    )
                    patterns.append(pattern)
            ...
            if 'timestamp' in df.columns:
                ...
                if len(unusual_hours) > 0:
                    pattern = DetectedPattern(
                        ...
                        entities_involved=df[df['hour'].isin(unusual_hours.index)]['entity_id'].tolist(),
    ...
    async def _detect_behavioral_patterns(self, data: Dict[str, Any]) -> List[DetectedPattern]:
                    ...
                    if len(user_logins) > 3:
                        ...
                        if any(hour < 6 or hour > 22 for hour in user_hours):
                            pattern = DetectedPattern(
                                ...
                                evidence={'unusual_hours': [h for h in user_hours if h < 6 or h > 22]},
                                risk_score=0.5,
                                timestamp=datetime.utcnow()
                            )
                            patterns.append(pattern)
            ...
            if 'session_duration' in df.columns:
                ...
                if len(long_sessions) > 0:
                    pattern = DetectedPattern(
                        ...
                        evidence={'long_sessions': long_sessions['session_duration'].tolist()},
                        risk_score=0.3,
                        timestamp=datetime.utcnow()
                    )
                    patterns.append(pattern)
    ...
    async def _detect_temporal_patterns(self, data: Dict[str, Any]) -> List[DetectedPattern]:
        ...
        try:
            ...
            if 'timestamp' in df.columns:
                ...
                if len(daily_counts) > 7:  # At least a week of data
                    ...
                    if std_daily > 0:
                        unusual_days = daily_counts[np.abs(daily_counts - mean_daily) > 2 * std_daily]
                ...
                off_peak_hours = hourly_counts[hourly_counts < hourly_counts.quantile(0.2)]
                ...
                if len(weekly_counts) == 7:  # Full week of data
                    weekend_activity = weekly_counts[weekly_counts.index.isin([5, 6])].sum()
                    weekday_activity = weekly_counts[weekly_counts.index.isin([0, 1, 2, 3, 4])].sum()
                    
                    if weekend_activity > weekday_activity * 0.8:  # High weekend activity
                        pattern = DetectedPattern(
                            ...
                            entities_involved=[],
                            evidence={'weekend_activity': weekend_activity, 'weekday_activity': weekday_activity},
                            risk_score=0.5,
                            timestamp=datetime.utcnow()
                        )
                        patterns.append(pattern)
    ...
    async def _detect_spatial_patterns(self, data: Dict[str, Any]) -> List[DetectedPattern]:
        """Detect geographic and spatial patterns."""
        try:
            patterns = []
            ...
            if 'location' in df.columns and 'entity_id' in df.columns:
                ...
                unusual_locations = location_frequencies[location_frequencies < location_frequencies.mean() * 0.5]
                ...
                if len(unusual_locations) > 0:
                    pattern = DetectedPattern(
                        ...
                        entities_involved=df[df['location'].isin(unusual_locations.index)]['entity_id'].tolist(),
            ...
            if 'latitude' in df.columns and 'longitude' in df.columns:
                ...
                for entity_id in df['entity_id'].unique():
                    entity_locations = df[df['entity_id'] == entity_id].sort_values('timestamp')
                        ...
                        if distances:
                            ...
                            if mean_distance > 1000:  # More than 1000 km average
                                pattern = DetectedPattern(
                                    ...
                                    evidence={'average_distance': mean_distance, 'distances': distances},
                                    risk_score=0.7,
                                    timestamp=datetime.utcnow()
                                )
                                patterns.append(pattern)
    ...
    async def _detect_network_patterns(self, data: Dict[str, Any]) -> List[DetectedPattern]:
        """Detect patterns in network structures."""
        try:
            patterns = []
            ...
            if 'nodes' in network_data and 'edges' in network_data:
                ...
                if density > 0.8:  # Very dense network
                    pattern = DetectedPattern(
                        ...
                        evidence={'density': density, 'nodes': G.number_of_nodes(), 'edges': G.number_of_edges()},
                        risk_score=0.5,
                        timestamp=datetime.utcnow()
                    )
                    patterns.append(pattern)
                ...
                if G.number_of_nodes() > 0:
                    ...
                    high_centrality_nodes = [
                        ...
                        if cent > np.mean(list(centrality.values())) + 2 * np.std(list(centrality.values()))
                    ]
                    ...
                    if high_centrality_nodes:
                        pattern = DetectedPattern(
                            ...
                            evidence={'centrality_scores': {node: centrality[node] for node in high_centrality_nodes}},
                            risk_score=0.6,
                            timestamp=datetime.utcnow()
                        )
                        patterns.append(pattern)
                    ...
                    if len(communities) > 1:
                        ...
                        if isolated_communities:
                            pattern = DetectedPattern(
                                ...
                                entities_involved=[node for comm in isolated_communities for node in comm],
                                evidence={'isolated_communities': [list(comm) for comm in isolated_communities]},
                                risk_score=0.3,
                                timestamp=datetime.utcnow()
                            )
                            patterns.append(pattern)
    ...
    async def _detect_anomaly_patterns(self, data: Dict[str, Any]) -> List[DetectedPattern]:
        """Detect anomaly patterns using machine learning."""
        try:
            patterns = []
                ...
                if len(X) > 10:  # Need sufficient data
                    ...
                    if len(anomaly_indices) > 0:
                        pattern = DetectedPattern(
                            ...
                            entities_involved=df.iloc[anomaly_indices]['entity_id'].tolist() if 'entity_id' in df.columns else [],
                            evidence={'anomaly_indices': anomaly_indices.tolist(), 'method': 'isolation_forest'},
                            risk_score=0.8,
                            timestamp=datetime.utcnow()
                        )
                        patterns.append(pattern)
                    ...
                    if len(X_scaled) > 5:
                        ...
                        if len(noise_indices) > 0:
                            pattern = DetectedPattern(
                                ...
                                entities_involved=df.iloc[noise_indices]['entity_id'].tolist() if 'entity_id' in df.columns else [],
                                evidence={'noise_indices': noise_indices.tolist(), 'method': 'dbscan'},
                                risk_score=0.6,
                                timestamp=datetime.utcnow()
                            )
                            patterns.append(pattern)
    ...
    async def _detect_sequential_patterns(self, data: Dict[str, Any]) -> List[DetectedPattern]:
        ...
        try:
            ...
            if 'sequence' in df.columns and 'entity_id' in df.columns:
                ...
                for entity_id in df['entity_id'].unique():
                    entity_sequences = df[df['entity_id'] == entity_id]['sequence'].tolist()
                    ...
                    if len(entity_sequences) > 2:
                        ...
                        if repeated_patterns:
                            pattern = DetectedPattern(
                                ...
                                entities_involved=[entity_id],
                                evidence={'repeated_patterns': repeated_patterns, 'pattern_counts': pattern_counts},
                                risk_score=0.5,
                                timestamp=datetime.utcnow()
                            )
                            patterns.append(pattern)
                        ...
                        if std_length > 0:
                            unusual_lengths = [
                                seq for seq, length in zip(entity_sequences, sequence_lengths)
                            ...
                            if unusual_lengths:
                                pattern = DetectedPattern(
                                    ...
                                    category=PatternCategory.LOW_RISK,
                                        ...
                                        entities_involved=[entity_id],
                                        evidence={'unusual_sequences': unusual_lengths, 'lengths': [len(seq) for seq in unusual_lengths]},
                                        risk_score=0.3,
                                        timestamp=datetime.utcnow()
                                    )
                                    patterns.append(pattern)
    ...
    async def _detect_correlation_patterns(self, data: Dict[str, Any]) -> List[DetectedPattern]:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'pattern_types_supported': [
                'transaction_patterns', 'behavioral_patterns', 'temporal_patterns',
                'spatial_patterns', 'network_patterns', 'anomaly_patterns',
                'sequential_patterns', 'correlation_patterns'
            ],
            'detection_methods_supported': [
                'statistical', 'machine_learning', 'rule_based', 'clustering',
                'sequence_analysis', 'graph_analysis', 'temporal_analysis', 'hybrid'
            ]
        }
