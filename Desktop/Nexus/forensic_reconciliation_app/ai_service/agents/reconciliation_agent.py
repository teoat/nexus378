...
class ReconciliationAgent:
    ...
    async def add_reconciliation_record(self, record: ReconciliationRecord) -> bool:
        ...
        try:
            ...
            # Normalize data
            record.normalized_data = await self._normalize_record_data(record.record_data)
    ...
    async def process_reconciliation(self, source_system: str, target_system: str, 
                                  record_type: str = None) -> List[MatchResult]:
        ...
        try:
            ...
            source_records = [
                r for r in self.reconciliation_records.values()
                if r.source_system == source_system and (not record_type or r.record_type == record_type)
            ]
            
            target_records = [
                r for r in self.reconciliation_records.values()
                if r.target_system == target_system and (not record_type or r.record_type == record_type)
            ]
            
            if not source_records or not target_records:
                self.logger.info(f"No records found for reconciliation: {source_system} -> {target_system}")
                return []
            
            self.logger.info(f"Processing reconciliation: {len(source_records)} source records vs {len(target_records)} target records")
            
            # Process each source record
            all_matches = []
            for source_record in source_records:
                matches = await self._find_matches_for_record(source_record, target_records)
            ...
            self.logger.info(f"Reconciliation completed: {len(all_matches)} matches found in {processing_time:.2f}s")
            return all_matches
    ...
    async def get_reconciliation_status(self, source_system: str, target_system: str) -> Dict[str, Any]:
        ...
        try:
            ...
            matched_count = sum(1 for r in source_records if r.reconciliation_status == "matched")
            unmatched_count = sum(1 for r in source_records if r.reconciliation_status == "unmatched")
            ...
            return {
                ...
                'match_rate': matched_count / len(source_records) if source_records else 0,
    ...
    async def _find_matches_for_record(self, source_record: ReconciliationRecord, 
                                     target_records: List[ReconciliationRecord]) -> List[MatchResult]:
        ...
        try:
            ...
            self.logger.error(f"Error finding matches for record {source_record.id}: {e}")
            return []
    
    async def _exact_match(self, source_record: ReconciliationRecord, 
                          target_record: ReconciliationRecord) -> Optional[MatchResult]:
    ...
    async def _hash_match(self, source_record: ReconciliationRecord, 
                         target_record: ReconciliationRecord) -> Optional[MatchResult]:
        """Perform hash-based matching between records."""
        try:
            # Check multiple hash algorithms
            for hash_alg in self.hash_algorithms:
                source_hash = self._generate_hash_with_algorithm(source_record.record_data, hash_alg)
                target_hash = self._generate_hash_with_algorithm(target_record.record_data, hash_alg)
    ...
    async def _fuzzy_match(self, source_record: ReconciliationRecord, 
                          target_record: ReconciliationRecord) -> Optional[MatchResult]:
        ...
        try:
            ...
            for field, source_value in source_record.normalized_data.items():
                if field in target_record.normalized_data:
                    ...
                    similarity = difflib.SequenceMatcher(None, source_str, target_str).ratio()
    ...
    def _generate_hash_with_algorithm(self, data: Dict[str, Any], algorithm: str) -> str:
    ...
    async def _cleanup_old_records(self):
        ...
        while True:
            try:
                ...
                # Remove old records
                old_records = [
                    record_id for record_id, record in self.reconciliation_records.items()
                ...
                if old_records:
                    self.logger.info(f"Cleaned up {len(old_records)} old reconciliation records")
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'match_rate': self.total_matches_found / self.total_records_processed if self.total_records_processed > 0 else 0
        }
