...
Examples:
  ...
  python todo_cli.py list
        ...
        config_parser = subparsers.add_parser(
            "config", help="Show or modify configuration"
        )
    ...
    async def _run_automation(self, args) -> int:
        ...
        self.automation = TodoAutomationSystem(max_concurrent_agents=args.max_agents)
    ...
    async def _run_with_progress(self):
        """Run automation with progress monitoring"""
        ...
        try:
            while not automation_task.done():
                ...
                print(
                    f"\r🔄 Progress: {progress['completed']}/{progress['total']} completed "
    ...
    def _print_text_todos(self, todos: list):
        """Print TODOs in text format"""
        print(f"\n📋 Found {len(todos)} TODOs:")
        print("=" * 60)

        for i, todo in enumerate(todos, 1):
            priority_emoji = (
                "🔴" if todo.priority >= 4 else "🟡" if todo.priority >= 3 else "🟢"
            )
