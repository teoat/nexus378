...
class HardwareTokenService:
    ...
    async def generate_challenge(self, user_id: str, token_id: str) -> Optional[str]:
        ...
        try:
            ...
            if token.status != "active":
                logger.warning(
                    f"Token {token_id} is not active (status: {token.status})"
                )
                return None
            ...
            challenge = HardwareChallenge(
                ...
                + timedelta(seconds=self.hardware_config["challenge_timeout"]),
    ...
    async def verify_response(self, user_id: str, challenge_id: str, response: str):
    ...
    async def _verify_token_response(
        self, token: HardwareToken, challenge: HardwareChallenge, response: str
    ) -> bool:
        """Verify response based on token type"""
        try:
            provider = self.token_providers.get(token.token_type)
    ...
    async def cleanup_expired_challenges(self):
        ...
        try:
            ...
            if expired_ids:
                logger.info(
                    f"Cleaned up {len(expired_ids)} expired hardware token challenges"
                )
    ...
    def get_system_status(self) -> Dict[str, any]:
        ...
        return {
            ...
            "active_tokens": len(
                [t for t in self.registered_tokens.values() if t.status == "active"]
            ),
            "active_challenges": len(self.active_challenges),
            "supported_types": [
                t.value for t in self.hardware_config["supported_types"]
            ],
            "status": "active",
        }
