...
class MFAManager:
    ...
    async def create_mfa_challenge(
        ...
        try:
            ...
            challenge = MFAChallenge(
                ...
                + timedelta(seconds=self.mfa_config["challenge_timeout"]),
            ...
            logger.info(
                f"Created MFA challenge {challenge.id} for user {user_id} ({mfa_type.value})"
            )
            return challenge
    ...
    async def _generate_challenge_data(
        ...
        try:
            if mfa_type == MFAType.TOTP:
                ...
                return await self.sms_service.generate_sms_code(
                    user_id, "+1234567890"
                )  # Default phone
    ...
    async def _verify_response(self, challenge: MFAChallenge, response: str) -> bool:
        ...
        try:
            if challenge.mfa_type == MFAType.TOTP:
                ...
                return await self.sms_service.verify_sms_code(
                    challenge.user_id, response
                )
            elif challenge.mfa_type == MFAType.HARDWARE:
                return await self.hardware_service.verify_response(
                    challenge.user_id, response
                )
            else:
    ...
    async def enable_mfa_for_user(
        self, user_id: str, mfa_type: MFAType, enabled: bool = True
    ):
