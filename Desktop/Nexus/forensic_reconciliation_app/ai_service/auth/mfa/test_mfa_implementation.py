...
async def test_mfa_manager():
    ...
    try:
        ...
        await mfa_manager.enable_mfa_for_user(user_id, mfa_manager.MFAType.HARDWARE, True)
        
        logger.info(f"✅ MFA types enabled for user {user_id}")
        
        # Test MFA challenge creation
        challenge = await mfa_manager.create_mfa_challenge(user_id, mfa_manager.MFAType.TOTP)
        if challenge:
        ...
        sms_challenge = await mfa_manager.create_mfa_challenge(user_id, mfa_manager.MFAType.SMS)
        if sms_challenge:
        ...
        hw_challenge = await mfa_manager.create_mfa_challenge(user_id, mfa_manager.MFAType.HARDWARE)
        if hw_challenge:
...
async def test_encryption_service():
    ...
    try:
        ...
        private_key_id, public_key_id = encryption_service.generate_user_key_pair(user_id, master_key_id)
        if private_key_id and public_key_id:
            logger.info(f"✅ User key pair generated: {private_key_id}, {public_key_id}")
        else:
        ...
        # Decrypt data
        decrypted_data = encryption_service.decrypt_data(ciphertext, master_key, iv, tag)
