...
class MFASystemTester:
    ...
    def test_totp_authenticator(self):
        ...
        try:
            ...
            assert code and len(code) == self.config.totp.digits, "Code generation failed"
    ...
    async def test_sms_authenticator(self):
        ...
        try:
            ...
            # Test code validation
            validation_result = self.mfa_manager.sms_auth.validate_code(phone_number, result.code)
            assert validation_result.success, "SMS code validation failed"
            
            # Test invalid code
            invalid_result = self.mfa_manager.sms_auth.validate_code(phone_number, "000000")
            ...
            assert not status['has_active_code'], "Code should be consumed after validation"
    ...
    def test_hardware_authenticator(self):
        ...
        try:
            ...
            challenge = self.mfa_manager.hardware_auth.generate_challenge("test_user_001", "test_token_001")
            ...
            expected_response = self.mfa_manager.hardware_auth.tokens["test_token_001"]._generate_expected_response(challenge, "test_user_001")
            
            result = self.mfa_manager.hardware_auth.validate_challenge_response(challenge, expected_response, "test_user_001")
            assert result.success, "Challenge response validation failed"
            
            # Test invalid response
            invalid_result = self.mfa_manager.hardware_auth.validate_challenge_response(challenge, "invalid_response", "test_user_001")
            ...
            self.test_results.append(("Hardware Token Authenticator", True, "All tests passed"))
            return True
    ...
    def test_mfa_setup(self):
        ...
        try:
            ...
            assert not setup_result.setup_complete, "Setup should not be complete initially"
    ...
    async def test_mfa_authentication(self):
        ...
        try:
            ...
            if 'sms' in setup_result.setup_data['methods']:
                ...
                # Verify SMS setup
                sms_success = await self.mfa_manager.verify_sms_setup(user_id, phone, sms_result.code)
    ...
    async def test_security_features(self):
        ...
        try:
            ...
            if current_code:
                ...
                assert session_data is None, "Session should be invalid after invalidation"
