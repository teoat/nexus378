...
class AgentCommunication:
    ...
    async def unregister_agent(self, agent_id: str) -> bool:
        ...
        try:
            if agent_id in self.message_queues:
                ...
                channels_to_remove = [
                    channel_id for channel_id, channel in self.communication_channels.items()
    ...
    async def send_message(self, message_type: MessageType, priority: MessagePriority,
                          source_agent: str, target_agents: List[str], subject: str,
                          content: Dict[str, Any], expiration: Optional[datetime] = None) -> str:
        ...
        try:
            ...
            message = AgentMessage(
                ...
                expiration=expiration or (datetime.utcnow() + timedelta(seconds=self.message_timeout))
            )
            ...
            if source_agent in self.message_queues:
                ...
                self.message_queues[source_agent].priority_queues[priority].append(message_id)
            ...
            for target_agent in target_agents:
                if target_agent in self.message_queues:
                    ...
                    self.message_queues[target_agent].priority_queues[priority].append(message_id)
            
            # Update statistics
            self.total_messages_sent += 1
            
            self.logger.info(f"Sent message: {message_id} from {source_agent} to {target_agents}")
            return message_id
    ...
    async def get_messages(self, agent_id: str, limit: int = 100, 
                          priority: Optional[MessagePriority] = None) -> List[AgentMessage]:
        ...
        try:
            if message_id in self.messages:
                ...
                if agent_id in message.target_agents:
                    ...
                    self.logger.info(f"Message {message_id} marked as read by {agent_id}")
                    return True
    ...
    async def create_communication_channel(self, source_agent: str, target_agent: str,
                                         channel_type: str = "direct") -> str:
        """Create a communication channel between agents."""
        try:
            # Validate agents
            if source_agent not in self.message_queues or target_agent not in self.message_queues:
        ...
        try:
            if channel_id in self.communication_channels:
                ...
                if channel.source_agent in self.message_routes:
                    if channel.target_agent in self.message_routes[channel.source_agent]:
                        self.message_routes[channel.source_agent].remove(channel.target_agent)
    ...
    async def broadcast_message(self, source_agent: str, subject: str, content: Dict[str, Any],
        ...
        try:
            ...
            self.logger.info(f"Broadcast message sent: {message_id} to {len(target_agents)} agents")
            return message_id
                    ...
                    for priority in [MessagePriority.EMERGENCY, MessagePriority.CRITICAL,
                        ...
                        while priority_queue:
                            ...
                            if message_id in self.messages:
                                ...
                                if message.expiration and datetime.utcnow() > message.expiration:
    ...
    async def _handle_task_request(self, message: AgentMessage, agent_id: str):
        ...
        try:
            ...
            self.logger.info(f"Processing task request: {message.subject} for agent {agent_id}")
    ...
    async def _handle_data_request(self, message: AgentMessage, agent_id: str):
        ...
        try:
            ...
            self.logger.info(f"Processing data request: {message.subject} for agent {agent_id}")
    ...
    async def _handle_status_update(self, message: AgentMessage, agent_id: str):
        ...
        try:
            ...
            self.logger.info(f"Processing status update: {message.subject} for agent {agent_id}")
    ...
    async def _handle_error_report(self, message: AgentMessage, agent_id: str):
        ...
        try:
            ...
            self.logger.info(f"Processing error report: {message.subject} for agent {agent_id}")
    ...
    async def _handle_broadcast(self, message: AgentMessage, agent_id: str):
        ...
        try:
            ...
            self.logger.info(f"Processing broadcast: {message.subject} for agent {agent_id}")
        ...
        while True:
            try:
                ...
                for message_id, message in self.messages.items():
                    if (message.expiration and current_time > message.expiration and
                        message.status == MessageStatus.PENDING):
                        message.status = MessageStatus.EXPIRED
                        expired_messages.append(message_id)
                
                if expired_messages:
                    self.logger.warning(f"Found {len(expired_messages)} expired messages")
        ...
        while True:
            try:
                ...
                if self.total_messages_sent > 0:
                    self.message_success_rate = self.total_messages_delivered / self.total_messages_sent
