...
class MessageQueue:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.default_message_size = config.get('default_message_size', 1024 * 1024)  # 1MB
        self.default_retention_period = config.get('default_retention_period', 3600)  # 1 hour
    ...
    def _initialize_default_queues(self):
        ...
        try:
            ...
            # Initialize queues
            self.queues[priority_queue_config.queue_id] = deque(maxlen=priority_queue_config.max_size)
            self.queues[fifo_queue_config.queue_id] = deque(maxlen=fifo_queue_config.max_size)
            self.queues[bulk_queue_config.queue_id] = deque(maxlen=bulk_queue_config.max_size)
    ...
    def _initialize_default_handlers(self):
        ...
        try:
            ...
            task_handler = MessageHandler(
                ...
                priority_levels=[MessagePriority.CRITICAL, MessagePriority.HIGH, MessagePriority.NORMAL],
            ...
            self.message_handlers[notification_handler.handler_id] = notification_handler
            ...
            self.logger.info(f"Initialized {len(self.message_handlers)} default message handlers")
            
        except Exception as e:
            self.logger.error(f"Error initializing default handlers: {e}")
    
    async def create_queue(self, queue_name: str, queue_type: QueueType,
                          max_size: int = None, enable_priority: bool = True) -> str:
    ...
    async def send_message(self, queue_id: str, message_type: MessageType,
                          priority: MessagePriority, sender_id: str,
                          recipient_id: str, payload: Any,
    ...
    async def receive_message(self, queue_id: str, timeout: float = None) -> Optional[Message]:
        ...
        try:
            ...
            if queue:
                ...
                self.logger.info(f"Received message {message.message_id} from queue {queue_id}")
    ...
    async def process_message(self, message: Message) -> bool:
        ...
        try:
            ...
            if not handlers:
                self.logger.warning(f"No handlers found for message type: {message.message_type}")
                return False
            ...
            if queue_id:
                ...
                if metrics.completed_messages > 0:
                    metrics.average_processing_time = (
                        (metrics.average_processing_time * (metrics.completed_messages - 1) + processing_time) /
            ...
            self.logger.info(f"Processed message {message.message_id}: {message.status.value}")
        ...
        while True:
            try:
                ...
                for message_id in expired_messages:
                    ...
                    if queue_id:
                        self.queue_metrics[queue_id].pending_messages -= 1
                        self.queue_metrics[queue_id].queue_size = len(self.queues[queue_id])
        ...
        while True:
            try:
                ...
                for queue_id, queue in self.queues.items():
                    ...
                    if metrics.processing_messages > 0 and metrics.average_processing_time > 5.0:
                        self.logger.warning(f"Queue {queue_id} has slow processing: {metrics.average_processing_time:.2f}s")
                    
                    # Check failure rate
                    if metrics.total_messages > 0:
                        failure_rate = metrics.failed_messages / metrics.total_messages
                        if failure_rate > 0.1:  # 10% failure rate
                            self.logger.warning(f"Queue {queue_id} has high failure rate: {failure_rate:.2%}")
    ...
    def get_queue_metrics(self, queue_id: str = None) -> Union[QueueMetrics, Dict[str, QueueMetrics]]:
