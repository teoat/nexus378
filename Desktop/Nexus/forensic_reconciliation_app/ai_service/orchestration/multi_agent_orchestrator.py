...
class MultiAgentOrchestrator:
    ...
    async def submit_workflow(self, workflow_type: WorkflowType, 
        ...
        try:
            ...
            self.workflow_queue = deque(sorted(self.workflow_queue, key=lambda x: x[0], reverse=True))
            
            self.logger.info(f"Submitted workflow: {workflow_id} ({workflow_type.value})")
            return workflow_id
        ...
        try:
            ...
            for agent_id, agent in self.agents.items():
                if (agent.status == AgentStatus.IDLE and 
                    all(cap in agent.capabilities for cap in step.required_capabilities)):
    ...
    async def _execute_workflow_step(self, workflow: WorkflowExecution, 
                                   step: WorkflowStep, agent_id: str):
        ...
        try:
            ...
            self.agent_workloads[agent_id] += step.estimated_duration / 3600  # Convert to hours
    ...
    async def _release_agent(self, agent_id: str):
        ...
        try:
            if agent_id in self.agents:
                ...
                # Reduce workload
                if agent_id in self.agent_workloads:
                    self.agent_workloads[agent_id] = max(0.0, self.agent_workloads[agent_id] - 0.1)
        ...
        try:
            for workflow in self.active_workflows.values():
                if agent_id in workflow.agents_assigned.values():
                    ...
                    steps_to_remove = [
                        step_id for step_id, assigned_agent_id in workflow.agents_assigned.items()
                    ...
                    if len(steps_to_remove) > 0:
                        workflow.status = "failed"
                        self.logger.warning(f"Workflow {workflow.id} failed due to agent {agent_id} removal")
