...
def get_tab_status():
    """Get the status of all 9 tabs"""
    
    # Check if collective worker processes are running
    try:
        result = subprocess.run(['ps', 'aux'], capture_output=True, text=True)
        if result.returncode == 0:
            lines = result.stdout.split('\n')
            collective_processes = [line for line in lines if 'collective_worker_processor.py' in line and 'grep' not in line]
...
def show_9_tab_dashboard():
    """Display the 9-tab system monitoring dashboard"""
    
    clear_screen()
    
    # Get system status
    active_workers = get_tab_status()
    ...
    print("🔧 Total Tabs: 9")
    print("👥 Active Collective Workers: " + ("🟢 RUNNING" if active_workers >= 8 else f"⚠️  {active_workers}/8"))
    ...
    tabs = [
        ("Tab 1", "Terminal 1 - Complex TODO Breakdown", "15 workers", "🟢 ACTIVE" if active_workers >= 1 else "🔴 INACTIVE"),
        ("Tab 2", "Terminal 2 - Micro-task Processing", "12 workers", "🟢 ACTIVE" if active_workers >= 2 else "🔴 INACTIVE"),
        ("Tab 3", "Terminal 3 - Worker Coordination", "10 workers", "🟢 ACTIVE" if active_workers >= 3 else "🔴 INACTIVE"),
        ("Tab 4", "Terminal 4 - Cache Management", "8 workers", "🟢 ACTIVE" if active_workers >= 4 else "🔴 INACTIVE"),
        ("Tab 5", "Terminal 5 - Progress Tracking", "6 workers", "🟢 ACTIVE" if active_workers >= 5 else "🔴 INACTIVE"),
        ("Tab 6", "Terminal 6 - Status Synchronization", "5 workers", "🟢 ACTIVE" if active_workers >= 6 else "🔴 INACTIVE"),
        ("Tab 7", "Terminal 7 - Error Handling", "4 workers", "🟢 ACTIVE" if active_workers >= 7 else "🔴 INACTIVE"),
        ("Tab 8", "Terminal 8 - Logging & Monitoring", "3 workers", "🟢 ACTIVE" if active_workers >= 8 else "🔴 INACTIVE"),
        ("Tab 9", "📊 SYSTEM MONITORING DASHBOARD", "📊 Active", "This tab - monitoring all terminals")
    ]
