...
class CollectiveMonitoringDashboard:
    ...
    def update_dashboard(self):
        ...
        try:
            ...
            if TODO_MASTER_AVAILABLE and task_registry:
                ...
                collective_processed = len(
                    [
                        t
                        for t in todos
                        if t.get("assigned_to_collective_processor", False)
                    ]
                )
    ...
    def display_dashboard(self):
        ...
        print("=" * 80)
        print(
            f"📅 System Running Since: {self.dashboard_data['system_start_time'].strftime('%Y-%m-%d %H:%M:%S')}"
        )
        ...
        print(
            f"⚡ Active Workers: {self.dashboard_data['collective_stats'].get('total_active_workers', 0)}"
        )
        print(
            f"📊 Worker Utilization: {self.dashboard_data['collective_stats'].get('worker_utilization', 0):.1f}%"
        )
        print()
        ...
        if self.dashboard_data["recent_activity"]:
            ...
            for activity in self.dashboard_data["recent_activity"][-5:]:  # Show last 5
                print(
                    f"🕒 {activity['time']}: {activity['action']} (Workers: {activity['workers_active']})"
                )
            print()
...
def demo_monitoring_dashboard():
    ...
    # Add demo processors
    for i in range(8):
        processor = CollectiveWorkerProcessor(
            max_workers=8 - i, min_batch_size=3, max_batch_size=50
        )
