...
class CorrectedTodoMasterReader:
    """Correctly reads and parses TODO_MASTER.md file for real work items"""

    def __init__(self, todo_master_path: Optional[str] = None):
        """Initialize with path to TODO_MASTER.md"""
        if todo_master_path:
            self.todo_master_path = Path(todo_master_path)
        else:
            # Auto-detect path relative to current location
    ...
    def read_todo_master(self) -> List[Dict[str, Any]]:
        """Read and parse TODO_MASTER.md file correctly"""
    ...
    def _parse_real_markdown_content(self, content: str) -> List[Dict[str, Any]]:
        """Parse the real TODO_MASTER.md format"""
        todos = []

        # Look for the actual TODO items in the real format
        # The real format has sections like:
    ...
    def _extract_real_todos_from_section(self, section: str) -> List[Dict[str, Any]]:
        """Extract TODO items from a section using the real format"""
        todos = []
    ...
    def _determine_real_priority(self, section_title: str, task_name: str) -> str:
        """Determine priority based on section and task context"""
    ...
    def _determine_real_duration(self, task_name: str, description: str) -> str:
        """Determine estimated duration based on task complexity"""
        task_lower = task_name.lower()
        desc_lower = description.lower()

        # Syntax errors are usually quick fixes
    ...
    def get_section_summary(self, todos: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Get summary of TODOs by section"""
        section_summary = {}
