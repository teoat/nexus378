...
class ForensicAnalysisAgent(AgentBase):
    ...
    def _can_handle_task(self, task) -> bool:
        ...
        return any(keyword in task.name.lower() or keyword in task.description.lower() 
    ...
    async def _execute_task(self, task):
        ...
        if task_registry.is_task_implemented(task.name, task.description):
            existing_agent = task_registry.get_implementing_agent(task.name, task.description)
...
async def create_example_agents(mcp_integration):
    ...
    data_agent = await mcp_integration.create_agent(
        "DataProcessingAgent", 
        ["data_cleaning", "data_transformation", "data_validation", "report_generation"]
    )
