...
class UnifiedTaskSystem:
    ...
    def _detect_task_conflict(self, task_id: str, worker_id: str) -> bool:
        ...
        for other_task in self.tasks.values():
            if (
                ...
                and other_task.status == TaskStatus.IN_PROGRESS
                and other_task.assigned_worker
                and other_task.assigned_worker != worker_id
            ):

                # Check for similarity (same capabilities, similar names)
                if (
                    set(other_task.required_capabilities)
                    & set(task.required_capabilities)
                    and self._calculate_task_similarity(task, other_task) > 0.7
                ):
                    return True
    ...
    def _calculate_task_similarity(self, task1: Task, task2: Task) -> float:
        ...
        name_similarity = self._string_similarity(
            task1.name.lower(), task2.name.lower()
        )
        capability_similarity = len(
            set(task1.required_capabilities) & set(task2.required_capabilities)
        ) / max(
            len(set(task1.required_capabilities) | set(task2.required_capabilities)), 1
        )
    ...
    def _update_worker_performance(self, worker_id: str, completed_task: Task):
        ...
        if completed_task.started_at and completed_task.completed_at:
            completion_time = (
                completed_task.completed_at - completed_task.started_at
            ).total_seconds()
    ...
    def _start_background_processes(self):
        ...
        self.worker_monitor_thread = threading.Thread(
            target=self._worker_monitoring_loop, daemon=True
        )
