...
class StatusMonitor:
                    ...
                    if task_key in task_names:
                        overlap_info = {
                            ...
                            "task1": {
                                ...
                                "name": task.name,
                                "agent": mcp_server.tasks[task_names[task_key]].agent_id
                            },
                            "task2": {
                                "id": task_id,
                                "name": task.name,
            ...
            for agent_id, agent in mcp_server.agents.items():
                for other_agent_id, other_agent in mcp_server.agents.items():
                    if agent_id != other_agent_id:
                        ...
                        common_capabilities = set(agent.capabilities).intersection(set(other_agent.capabilities))
                        if len(common_capabilities) > 0:
                            ...
                            for task_id in agent.current_tasks:
                                if task_id in mcp_server.tasks:
                                    ...
                                    if any(cap in other_agent.capabilities for cap in task.required_capabilities):
                                        overlap_info = {
    ...
    async def _update_performance_metrics(self):
        ...
        try:
            ...
            failed_tasks = len([t for t in mcp_server.tasks.values() if t.status.value == "failed"])
            ...
            self.performance_metrics.update({
                ...
                "active_agents": len([a for a in mcp_server.agents.values() if a.status == "available"]),
    ...
    async def _generate_status_report(self):
        ...
        try:
            ...
            # Log report summary
            logger.info(f"Status report generated - Health: {report['system_health']}, "
    ...
    def _get_todo_status_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "in_progress": registry_summary.get("by_status", {}).get("in_progress", 0),
                "completed": registry_summary.get("by_status", {}).get("completed", 0),
                "by_priority": registry_summary.get("by_priority", {})
            }
        except Exception:
            return {"total": 0, "pending": 0, "in_progress": 0, "completed": 0, "by_priority": {}}
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        try:
            ...
            if len(self.overlap_alerts) > 3:
                recommendations.append("High number of overlap alerts detected. Review agent assignments and task distribution.")
            
            # Check for low completion rates
            if self.performance_metrics.get("completion_rate", 0) < 70:
                recommendations.append("Low task completion rate. Consider adding more agents or reviewing task complexity.")
            
            # Check for agent availability
            available_agents = len([a for a in mcp_server.agents.values() if a.status == "available"])
            if available_agents == 0:
                recommendations.append("No available agents. Check agent health and restart if necessary.")
            
            # Check for priority TODO distribution
            todo_summary = self._get_todo_status_summary()
            if todo_summary.get("pending", 0) > 5:
                recommendations.append("High number of pending priority TODOs. Consider agent capacity planning.")
    ...
    def export_status_report(self, filepath: str = None) -> str:
        ...
        if not filepath:
            filepath = f"mcp_status_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
