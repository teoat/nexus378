...
from core.mcp_server import mcp_server, TaskStatus, TaskPriority
...
class DemoAgent:
    """Demo agent that can claim and work on tasks"""
    ...
    async def register(self) -> bool:
        ...
        success = await mcp_server.register_agent(
            self.agent_id, 
            self.name, 
            self.capabilities
        )
    ...
    async def claim_available_tasks(self) -> List[str]:
        """Claim available tasks that match capabilities"""
        available_tasks = await mcp_server.get_available_tasks(self.agent_id)
        ...
        for task in available_tasks:
            ...
            if await mcp_server.claim_task(self.agent_id, task.id):
                ...
                logger.warning(f"❌ Agent {self.name} failed to claim task: {task.name}")
        
        return claimed_tasks
    
    async def work_on_tasks(self, work_duration: float = 2.0):
        """Simulate working on claimed tasks"""
        if not self.current_tasks:
            logger.info(f"🤷 Agent {self.name} has no tasks to work on")
            return
        
        logger.info(f"🚧 Agent {self.name} working on {len(self.current_tasks)} tasks...")
        await asyncio.sleep(work_duration)
        
        # Complete all tasks
        for task_id in self.current_tasks[:]:  # Copy list to avoid modification during iteration
            await mcp_server.complete_task(self.agent_id, task_id, {
    ...
    async def heartbeat(self):
        ...
        await mcp_server.heartbeat(self.agent_id)


async def demonstrate_mcp_functionality():
    """Demonstrate MCP server functionality with multiple agents"""
    
    logger.info("🚀 Starting MCP Server Demo")
    
    # Create demo agents with different capabilities
    agents = [
        DemoAgent("agent_001", "Database Specialist", [
            "database_setup", "olap_configuration", "performance_optimization"
        ]),
        DemoAgent("agent_002", "Security Expert", [
            "security", "authentication", "mfa_implementation", "encryption", "key_management"
        ]),
        DemoAgent("agent_003", "Python Developer", [
            "python_development", "algorithm_implementation", "load_balancing",
            "workflow_management", "dependency_resolution"
        ]),
        DemoAgent("agent_004", "ML Engineer", [
            "python_development", "machine_learning", "algorithm_implementation",
            "graph_algorithms", "fraud_detection"
        ]),
        DemoAgent("agent_005", "Compliance Specialist", [
...
async def show_task_details():
    """Show detailed information about all tasks"""
