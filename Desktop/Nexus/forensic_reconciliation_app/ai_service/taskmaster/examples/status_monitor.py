...
class StatusMonitor:
    ...
    async def get_comprehensive_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "agents": agent_info,
                "overall_health": self._calculate_health_score(system_status, priority_summary)
            }
    ...
    def _calculate_health_score(self, system_status: Dict[str, Any], priority_summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def update_task_progress(self, task_id: str, progress: float, status_update: str = None) -> bool:
        ...
        try:
            success = await self.mcp_server.update_task_progress(task_id, progress, status_update)
            if success:
        ...
        try:
            ...
            if task_status:
                ...
                if task_status.get("agent_id"):
                    agent_workload = await self.mcp_server.get_agent_workload(task_status["agent_id"])
    ...
    async def generate_status_report(self) -> str:
        ...
        try:
            ...
            report = []
            report.append("=" * 80)
            report.append("🔍 FORENSIC RECONCILIATION + FRAUD PLATFORM - STATUS REPORT")
            report.append("=" * 80)
            ...
            report.append(f"   Overall Score: {health.get('score', 0)}/100 - {health.get('status', 'Unknown')}")
            ...
            report.append(f"   Critical: {todos.get('by_priority', {}).get('critical', 0)}")
            ...
            for phase, data in phase_summary.items():
                if isinstance(data, dict) and "total_tasks" in data:
                    ...
                    report.append(f"     Tasks: {data['total_tasks']} | Completed: {data['completed']} | In Progress: {data['in_progress']} | Pending: {data['pending']}")
                    report.append(f"     Estimated Hours: {data['estimated_hours']:.1f}")
            report.append("")
            ...
            if agents:
                ...
                for agent_id, agent_data in agents.items():
                    if isinstance(agent_data, dict):
                        ...
                        report.append(f"   {name} ({agent_id}): {task_count} tasks | Status: {status}")
                report.append("")
            ...
            report.append(f"   Tasks In Progress: {system_status.get('in_progress_tasks', 0)}")
            report.append(f"   Tasks Completed: {system_status.get('completed_tasks', 0)}")
            report.append(f"   Tasks Failed: {system_status.get('failed_tasks', 0)}")
            report.append(f"   Available Agents: {system_status.get('available_agents', 0)}")
            report.append("")
...
async def main():
    ...
    logger.info(f"System Status: {status.get('overall_health', {}).get('status', 'Unknown')}")
