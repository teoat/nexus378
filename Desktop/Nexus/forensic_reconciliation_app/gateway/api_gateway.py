...
class APIGateway:
    ...
    async def create_api_route(self, base_path: str, service_name: str, endpoints: List[Dict[str, Any]],
                              authentication: AuthenticationType, rate_limiting: bool = True,
                              cors_enabled: bool = True) -> str:
            ...
            for route in self.routes.values():
                for endpoint in route.endpoints:
                    ...
                    if endpoint.method == HTTPMethod.GET:
                        self.app.router.add_get(full_path, self._create_endpoint_handler(endpoint))
                    elif endpoint.method == HTTPMethod.POST:
                        self.app.router.add_post(full_path, self._create_endpoint_handler(endpoint))
                    elif endpoint.method == HTTPMethod.PUT:
                        self.app.router.add_put(full_path, self._create_endpoint_handler(endpoint))
                    elif endpoint.method == HTTPMethod.DELETE:
                        self.app.router.add_delete(full_path, self._create_endpoint_handler(endpoint))
                    elif endpoint.method == HTTPMethod.PATCH:
                        self.app.router.add_patch(full_path, self._create_endpoint_handler(endpoint))
                    elif endpoint.method == HTTPMethod.OPTIONS:
                        self.app.router.add_options(full_path, self._create_endpoint_handler(endpoint))
                    elif endpoint.method == HTTPMethod.HEAD:
                        self.app.router.add_head(full_path, self._create_endpoint_handler(endpoint))
        ...
        async def handler(request):
            ...
            try:
                ...
                if endpoint.handler_function == 'health_check':
                    ...
                    response_data = {'message': 'Handler not implemented', 'status': 'error'}
                
                # Create API response
                response_time = (datetime.utcnow() - start_time).total_seconds()
                api_response = await self._create_api_response(api_request, response_data, response_time)
    ...
    async def _create_api_request(self, request: web.Request, endpoint: APIEndpoint) -> APIRequest:
    ...
    async def _create_api_response(self, api_request: APIRequest, response_data: Any, response_time: float) -> APIResponse:
    ...
    async def _cleanup_old_requests(self):
        ...
        while True:
            try:
                ...
                old_requests = [
                    req_id for req_id, request in self.api_requests.items()
                ...
                old_responses = [
                    resp_id for resp_id, response in self.api_responses.items()
                ...
                if old_requests or old_responses:
                    self.logger.info(f"Cleaned up {len(old_requests)} old requests and {len(old_responses)} old responses")
