"""
JWT Authentication System for Forensic Reconciliation Platform
...
class JWTAuthManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        """Initialize the JWT Auth Manager."""
        self.config = config
        self.secret_key = config.get('jwt_secret', 'your_super_secret_jwt_key_change_this')
        self.algorithm = config.get('jwt_algorithm', 'HS256')
        self.access_token_expiry = timedelta(hours=config.get('jwt_expires_in_hours', 24))
        self.refresh_token_expiry = timedelta(days=config.get('jwt_refresh_expires_in_days', 7))
        ...
        self.password_require_special_chars = config.get('password_require_special_chars', True)
        
        # MFA configuration
        self.mfa_enabled = config.get('mfa_enabled', True)
        self.mfa_issuer = config.get('mfa_issuer', 'Forensic Reconciliation Platform')
    ...
    def authenticate_user(self, username: str, password: str, mfa_token: Optional[str] = None) -> AuthResult:
    ...
    def validate_token(self, token: str) -> AuthResult:
        ...
        try:
            ...
            if not self._is_token_active(user.id, token):
                return AuthResult(
                    status=AuthStatus.INVALID,
                    message="Token not in active sessions"
                )
    ...
    def _is_token_active(self, user_id: str, token: str) -> bool:
        """Check if a token is active for a user."""
        if user_id in self.active_sessions:
    ...
    def cleanup_expired_sessions(self):
        ...
        if expired_refresh or expired_sessions:
            logger.info(f"Cleaned up {len(expired_refresh)} expired refresh tokens and {len(expired_sessions)} expired sessions")
