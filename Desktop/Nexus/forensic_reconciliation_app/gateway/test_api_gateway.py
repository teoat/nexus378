...
async def test_reconciliation_process_endpoint(cli):
    ...
    with patch.object(
        cli.server.app["gateway"].reconciliation_agent,
        ...
        resp = await cli.post(
            "/api/v1/ai/reconciliation/process", json={"records": [{"id": "1"}]}
        )
...
async def test_fraud_analyze_endpoint(cli):
    ...
    with patch.object(
        cli.server.app["gateway"].fraud_agent,
        ...
        resp = await cli.post(
            "/api/v1/ai/fraud/analyze", json={"transactions": [{"id": "1"}]}
        )
        assert resp.status == 200
        json_resp = await resp.json()
        assert json_resp["success"] is True
...
async def test_nlp_process_endpoint(cli):
    ...
    with (
        patch.object(
            cli.server.app["gateway"].nlp_processor,
            "process_document",
            new_callable=AsyncMock,
        ) as mock_process_doc,
        patch.object(
        ...
        resp = await cli.post(
            "/api/v1/ai/nlp/process", json={"content": "test content"}
        )
        assert resp.status == 200
        json_resp = await resp.json()
...
async def test_ocr_process_endpoint(cli):
    ...
    with patch.object(
        cli.server.app["gateway"].ocr_processor,
        "process_document",
        new_callable=AsyncMock,
    ) as mock_process:
        ...
        resp = await cli.post(
            "/api/v1/ai/ocr/process", json={"document_path": "/path/to/doc"}
        )
        assert resp.status == 200
        json_resp = await resp.json()
