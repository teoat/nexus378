...
class RoundRobinLoadBalancer:
        ...
        if is_healthy:
            ...
            if server.current_failures >= server.max_failures:
                if server.status != ServerStatus.UNHEALTHY:
                    ...
                    logger.warning(
                        f"Server {server.id} is now unhealthy (failures: {server.current_failures})"
                    )
            else:
                logger.debug(
                    f"Server {server.id} health check failed (failures: {server.current_failures})"
                )
    ...
    def record_request_result(
        self, server_id: str, success: bool, response_time: float = 0.0
    ):
    ...
    def get_load_balancer_statistics(self) -> Dict[str, Any]:
        ...
        for status in ServerStatus:
            status_counts[status.value] = len(
                [s for s in self.servers.values() if s.status == status]
            )
