...
@dataclass
class SystemConfig:
    """System-wide configurationSystem-wide configuration"""
...
CONFIGURATIONS = {
    "development": SystemConfig(
        max_concurrent_agents=3, log_level="DEBUG", processing_timeout=60.0
    ),
    "testing": SystemConfig(
        max_concurrent_agents=2, log_level="INFO", processing_timeout=30.0
    ),
    "production": SystemConfig(
        max_concurrent_agents=20, log_level="WARNING", processing_timeout=600.0
    ),
}

def get_config(environment: str = "development") -> SystemConfig:
    """Get configuration for specified environmentGet configuration for specified environment"""
    return CONFIGURATIONS.get(environment, DEFAULT_SYSTEM_CONFIG)

def get_agents_config() -> List[AgentConfig]:
    """Get default agent configurationsGet default agent configurations"""
    return DEFAULT_AGENTS.copy()

def load_config_from_file(config_path: str) -> Dict[str, Any]:
    """Load configuration from JSON fileLoad configuration from JSON file"""
    import json
...
def save_config_to_file(config: Dict[str, Any], config_path: str):
    """Save configuration to JSON fileSave configuration to JSON file"""
    import json
