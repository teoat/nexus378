...
class TestPluginManager(unittest.TestCase):
    ...
    def setUp(self):
        """Set up a temporary directory for test plugins and add it to sys.path.Set up a temporary directory for test plugins and add it to sys.path."""
...
class InvalidPlugin:
    ...
    def tearDown(self):
        """Remove the temporary directory and clean up sys.path.Remove the temporary directory and clean up sys.path."""
        sys.path.pop(0)
        shutil.rmtree(self.test_plugin_dir)

    def test_discover_and_load_valid_plugin(self):
        """Test that a valid plugin is discovered and loaded correctly.Test that a valid plugin is discovered and loaded correctly."""
    ...
    def test_ignore_invalid_and_broken_plugins(self):
        """Test that invalid or broken files do not crash the manager.Test that invalid or broken files do not crash the manager."""
        manager = PluginManager(plugin_folder=self.test_plugin_dir)
    ...
    def test_shutdown_calls_unload(self):
        """Test that the shutdown method calls unload on loaded plugins.Test that the shutdown method calls unload on loaded plugins."""
        manager = PluginManager(plugin_folder=self.test_plugin_dir)
        manager.discover_and_load_plugins()
