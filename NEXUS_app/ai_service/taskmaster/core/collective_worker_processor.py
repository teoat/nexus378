...
class CollectiveWorkerProcessor:
    ...
    def _initialize_workers(self):
        ...
        logger.info(
            f"Initialized {len(self.workers)} workers: {list(self.workers.keys())}"
        )

    def start_collective_processing_loop(self, interval=10):

            f"Starting collective processing loop with {interval} second intervals (optimized for 32 workers)"
        )
        ...
        try:
            while True:
                ...
                if available_todos:
                    ...
                    # Mark work items as in-progress - Optimized for 32 workers
                    marked_items = self._mark_work_items_in_progress(available_todos)

                    if marked_items:
                        # Process work items collectively with enhanced coordination
                        ...
                        logger.info(
                            f"Processed batch of {len(marked_items)} work items with 32 workers"
                        )
                    else:
    ...
    def _scan_for_available_work(self):
            ...
            # Use queue manager to determine if we should process
            if not self.queue_manager.should_process(available_todos):
                logger.info(
                    f"Not enough TODOs to process. Need {self.queue_manager.min_todos}, have {len(available_todos)}"
                )
                return []

            # Get processing batch (max 20)
            processing_batch = self.queue_manager.get_processing_batch(available_todos)
    ...
    def _mark_work_items_in_progress(self, work_items):
            ...
            wid for wid, status in self.worker_status.items() if status == "idle"
        ]

        if not available_workers:
            logger.warning("No available workers for work assignment")
            return []

        # Use queue manager to distribute work evenly
        marked_items = self.queue_manager.distribute_work_evenly(
            work_items, available_workers
        )

        # Update registry and worker status
        for item in marked_items:
            worker_id = item["assigned_worker"]
            self.todo_registry.update_todo_status(item["id"], "in-progress", worker_id)
            self.worker_status[worker_id] = "processing"
            self.workers[worker_id]["current_task"] = item["id"]

            logger.info(
                f"Marked work item {item['id']} as in-progress, assigned to {worker_id}"
            )
