#!/usr/bin/env python3
MCP Server Orchestrator for Nexus
...
class MCPServerInfo:
    ...
    async def _start_server(
        self, server_id: str, server_info_override: Optional[Dict] = None
    ) -> bool:
    ...
    async def _find_idle_agent_server(self) -> Optional[str]:
                ...
                if server_status.status == "running":
                    ...
                    server_status.uptime = str(uptime).split(".")[
                        0
                    ]  # Remove microseconds
                    ...
                    if (
                        server_status.health_status == "healthy"
                        and server_status.progress > 50.0
                    ):
                        server_status.health_status = "optimal"
                    elif (
                        server_status.health_status == "optimal"
                        and server_status.progress > 75.0
                    ):
    ...
    async def _log_system_status(self):
                ...
                [s for s in self.server_status.values() if s.status == "running"]
            )
            total_servers = len(self.server_status)
            overall_progress = (
                sum(s.progress for s in self.server_status.values()) / total_servers
                if total_servers > 0
                else 0
            )

            logger.info(
                f"System Status: {running_servers}/{total_servers} servers running, "
    ...
    def get_orchestrator_status(self) -> Dict[str, Any]:
            ...
            "stopped_servers": len(
                [s for s in self.server_status.values() if s.status == "stopped"]
            ),
            "failed_servers": len(
                [s for s in self.server_status.values() if s.status == "failed"]
            ),
            "overall_progress": (
                sum(s.progress for s in self.server_status.values())
                / len(self.server_status)
    ...
    def get_mcp_system_summary(self) -> Dict[str, Any]:
                ...
                s for s in self.server_status.values() if s.priority == "CRITICAL"
            ]
            high_priority_servers = [
                s for s in self.server_status.values() if s.priority == "HIGH"
            ]
            normal_priority_servers = [
                s for s in self.server_status.values() if s.priority == "NORMAL"
            ]

            completed_servers = [
                s
                for s in self.server_status.values()
                if s.mcp_status == "MCP_COMPLETED"
            ]
            in_progress_servers = [
                s for s in self.server_status.values() if s.mcp_status == "MCP_TRACKED"
            ]
            ...
            return {
                ...
                "progress_summary": {
                    ...
                    "critical_progress": (
                        sum(s.progress for s in critical_servers)
                        / len(critical_servers)
                        if critical_servers
                        else 0
                    ),
                    "high_priority_progress": (
                        sum(s.progress for s in high_priority_servers)
                        / len(high_priority_servers)
                        if high_priority_servers
                        else 0
                    ),
                    "normal_priority_progress": (
                        sum(s.progress for s in normal_priority_servers)
                        / len(normal_priority_servers)
                        if normal_priority_servers
                        else 0
                    ),
                },
                "health_summary": {
    ...
    try:
        ...
        if success:
            ...
            while orchestrator.orchestrator_status == "running":
                ...
                print(
                    f"🔄 Running Servers: {status['running_servers']}/{status['total_servers']}"
                )
