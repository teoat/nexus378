...
@dataclass
class MFAToken:
    """MFA token data structureMFA token data structure"""
...
@dataclass
class MFAChallenge:
    """MFA challenge response structureMFA challenge response structure"""
...
class MFAAuthenticator:
    """Main MFA authentication handlerMain MFA authentication handler"""
    ...
    def generate_totp_secret(self, user_id: str) -> str:
        """Generate a new TOTP secret for a userGenerate a new TOTP secret for a user"""
    ...
    def generate_sms_code(self, user_id: str, phone_number: str) -> Tuple[str, str]:
        """Generate a 6-digit SMS code for a userGenerate a 6-digit SMS code for a user"""
    ...
    def verify_totp(self, secret: str, token: str) -> Tuple[bool, str]:
        """Verify a TOTP tokenVerify a TOTP token"""
        ...
        for i in range(-self.totp_window, self.totp_window + 1):
            if (
                self._generate_totp_token(mfa_token.secret, current_time + i * 30)
                == token
            ):
        ...
        return (
            False,
            f"Invalid TOTP token. {mfa_token.attempts_remaining} attempts remaining",
        )

    def verify_sms_code(self, challenge_id: str, code: str) -> Tuple[bool, str]:
        """Verify an SMS codeVerify an SMS code"""
    ...
    def _generate_totp_token(self, secret: str, timestamp: int) -> str:
        """Generate TOTP token for a given timestamp (simplified)Generate TOTP token for a given timestamp (simplified)"""
    ...
    def create_mfa_jwt(self, user_id: str, mfa_type: str) -> str:
        """Create a JWT token after successful MFA verificationCreate a JWT token after successful MFA verification"""
        payload = {
            "user_id": user_id,
            "mfa_type": mfa_type,
            "mfa_verified": True,
    ...
    def verify_mfa_jwt(self, token: str) -> Tuple[bool, Dict[str, Any]]:
        """Verify an MFA JWT tokenVerify an MFA JWT token"""
        try:
    ...
    def revoke_mfa_token(self, secret: str) -> bool:
        """Revoke an MFA tokenRevoke an MFA token"""
    ...
    def cleanup_expired(self) -> int:
        """Clean up expired tokens and challengesClean up expired tokens and challenges"""
        now = datetime.utcnow()
        expired_tokens = [
            k for k, v in self.active_tokens.items() if v.expires_at < now
        ]
        expired_challenges = [
            k for k, v in self.active_challenges.items() if v.expires_at < now
        ]
...
mfa_authenticator = MFAAuthenticator(
    secret_key="your-secret-key-change-in-production", totp_window=2, max_attempts=3
)
