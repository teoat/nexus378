...
@dataclass
class SMSCode:
    """SMS code data structureSMS code data structure"""
    code: str
    user_id: str
    phone_number: str
    created_at: datetime
...
class SMSService:
    """SMS service for MFA authenticationSMS service for MFA authentication"""
    ...
    def generate_sms_code(self, user_id: str, phone_number: str) -> Tuple[str, str]:
        """Generate a new SMS code for a userGenerate a new SMS code for a user"""
    ...
    def resend_sms_code(self, code_id: str) -> Tuple[bool, str]:
        """Resend SMS code (generate new code with same ID)Resend SMS code (generate new code with same ID)"""
        if code_id not in self.active_codes:
    ...
    def _send_sms(self, phone_number: str, code: str) -> bool:
        """Send SMS with code (production implementation would use SMS provider)Send SMS with code (production implementation would use SMS provider)"""
        try:
    ...
    def get_code_status(self, code_id: str) -> Optional[Dict]:
        """Get status of an SMS codeGet status of an SMS code"""
        if code_id not in self.active_codes:
            return None
    ...
    def revoke_code(self, code_id: str) -> bool:
        """Revoke an SMS codeRevoke an SMS code"""
    ...
    def cleanup_expired(self) -> int:
        """Clean up expired SMS codesClean up expired SMS codes"""
        now = datetime.utcnow()
    ...
    def get_user_active_codes(self, user_id: str) -> list:
        """Get all active codes for a userGet all active codes for a user"""
        now = datetime.utcnow()
        user_codes = []
