...
class TaskBreakdownAnalyzer:
    ...
    async def analyze_and_breakdown_tasks(self):
        ...
        for i, task in enumerate(summary["tasks"], 1):
            ...
            if task_name in self.complex_task_definitions:
                ...
                for j, subtask in enumerate(subtasks, 1):
                    ...
                    print(
                        f"         Capabilities: {', '.join(subtask['capabilities'])}"
                    )
                ...
                breakdown_results.append(
                    {"task": task_name, "subtasks": [], "total_subtasks": 0}
                )
        ...
        print(
            f"Complex Tasks: {len([r for r in breakdown_results if r['total_subtasks'] > 0])}"
        )
        print(
            f"Simple Tasks: {len([r for r in breakdown_results if r['total_subtasks'] == 0])}"
        )
        ...
        print(
            f"📄 Generated {total_subtasks} actionable subtasks from {len(summary['tasks'])} complex tasks"
        )
    ...
    def generate_simple_todo_list(self, breakdown_results):
        ...
        for result in breakdown_results:
            if result["total_subtasks"] > 0:
                ...
                for subtask in result["subtasks"]:
                    print(
                        f"   {todo_counter}. {subtask['name']} ({subtask['duration']})"
                    )
                    todo_counter += 1
            else:
                print(f"\n🎯 {result['task']}:")
